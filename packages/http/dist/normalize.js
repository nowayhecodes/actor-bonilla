import { DEFAULT_TIMEOUT, DEFAULT_RETRY, DEFAULT_HOOKS, defaultRetryDelay, } from './defaults.js';
import { MemoryCache } from './cache.js';
import { assertRetryOptions, assertTimeoutOptions, assertPaginationOptions, } from './validation.js';
// ─── URL helpers ─────────────────────────────────────────────────────────────
export function resolveUrl(url, prefixUrl) {
...
export function applySearchParams(url, searchParams) {
    ...
    const usp = searchParams instanceof URLSearchParams
        ? searchParams
...
function normalizeRetry(retry) {
    ...
    if (retry === undefined || retry === null) {
        ...
        if (retry.limit !== undefined)
            base.limit = retry.limit;
        if (retry.methods)
            base.methods = retry.methods;
        if (retry.statusCodes)
            base.statusCodes = retry.statusCodes;
        if (retry.errorCodes)
            base.errorCodes = retry.errorCodes;
        if (retry.calculateDelay)
            base.calculateDelay = retry.calculateDelay;
        if (retry.backoffLimit !== undefined)
            base.backoffLimit = retry.backoffLimit;
        if (retry.maxRetryAfter !== undefined)
            base.maxRetryAfter = retry.maxRetryAfter;
    }
...
export function normalizeOptions(base, override = {}) {
    ...
    if (retry.calculateDelay === DEFAULT_RETRY.calculateDelay) {
        ...
        retry.calculateDelay = (obj) => {
            ...
            return Math.min(userCalc({ ...obj, computedValue: computed }), retry.backoffLimit);
        };
    }
    ...
    if (merged.pagination !== undefined) {
        assertPaginationOptions(merged.pagination);
    }
    return {
        url,
        method: merged.method?.toUpperCase() ?? 'GET',
        headers,
        body: merged.body,
        json: merged.json,
        form: merged.form,
...
export function mergeOptionObjects(a, b) {
    const merged = { ...a, ...b };
    ...
    if (a.hooks || b.hooks) {
        ...
        for (const key of allKeys) {
            const aArr = (a.hooks?.[key] ?? []);
            const bArr = (b.hooks?.[key] ?? []);
            merged.hooks[key] = [...aArr, ...bArr];
        }
    }
    return merged;
}
