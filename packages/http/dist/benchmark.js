...
async function bench(name, iterations, warmup, fn) {
...
function printHeader(title) {
    ...
    console.log(`  ${title}`);
    console.log(line);
    console.log(`  ${'name'.padEnd(44)} ${'avg'.padStart(14)} ${'p50'.padStart(14)} ${'p95'.padStart(14)} ${'p99'.padStart(14)} ${'ops/s'.padStart(13)}`);
    console.log(line);
}
// ─── Mock server ─────────────────────────────────────────────────────────────
const JSON_BODY = JSON.stringify({ id: 1, name: 'actor-bonilla', value: Math.PI });
const LARGE_BODY = JSON.stringify(Array.from({ length: 100 }, (_, i) => ({ id: i, data: 'x'.repeat(64) })));
...
async function main() {
    ...
    const r_client_conc = await bench(`HttpClient ×${CONCURRENCY} concurrent`, 20, 3, async () => {
        await Promise.all(Array.from({ length: CONCURRENCY }, () => client.getJson(`${BASE}/json`)));
    });
