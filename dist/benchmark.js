...
// ============================================================================
async function benchPingPong(totalMessages) {
    ...
    return new Promise((resolve) => {
        ...
        let start;
        let pong;
        const ping = system.actorOf(props((msg, ctx) => {
            if (msg === 'start') {
            ...
            if (msg === 'pong') {
                ...
                if (count < half) {
                    ...
                    console.log(`  Throughput: ${formatNum(Math.round(throughput))} msg/sec`);
                    system.terminate().then(resolve);
                }
            }
        }), 'ping');
        pong = system.actorOf(props((msg, ctx) => {
...
// ============================================================================
async function benchFanOut(numWorkers, messagesPerWorker) {
    ...
    return new Promise((resolve) => {
        ...
        for (let i = 0; i < numWorkers; i++) {
            workers.push(system.actorOf(props((msg) => {
...
// ============================================================================
async function benchChain(chainLength, totalMessages) {
    ...
    return new Promise((resolve) => {
        ...
        let next = system.actorOf(props((msg) => {
            if (typeof msg === 'number') {
                received++;
                if (received >= totalMessages) {
                    ...
                    console.log(`  Throughput: ${formatNum(Math.round(throughput))} hops/sec`);
                    system.terminate().then(resolve);
                }
            }
        }), 'tail');
...
// ============================================================================
async function benchMailboxFlood(totalMessages) {
    ...
    return new Promise((resolve) => {
        let count = 0;
        const start = performance.now();
        const actor = system.actorOf(props((msg) => {
            if (typeof msg === 'number') {
                count++;
