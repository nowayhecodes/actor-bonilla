...
import type { EventClassifier, EventSubscriber, DeadLetter } from './types.js';
...
export class EventStream {
  private subscribers = new Map<EventClassifier, Set<EventSubscriber<any>>>();
  ...
  publish<T>(classifier: EventClassifier, event: T): void {
    ...
    for (const sub of subs) {
      try {
        ...
        console.error(
          `[actor-bonilla] Error in event subscriber for "${String(classifier)}":`,
          e
        );
      }
    }
...
export const DEAD_LETTER_CHANNEL: EventClassifier =
  Symbol.for('actor-bonilla.deadLetters');
export const LOG_CHANNEL: EventClassifier = Symbol.for('actor-bonilla.log');
export const LIFECYCLE_CHANNEL: EventClassifier =
  Symbol.for('actor-bonilla.lifecycle');
